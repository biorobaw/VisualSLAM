...
class SeqSLAM():
    ...
    def preprocessing(params):
        ...
        for i in (params.dataset.imageIndices):
            ...
            if params.DO_RESIZE:
                ...
                img = img.resize((params.downsample.size[1], params.downsample.size[0]), params.downsample.method)
            ...
            for j in range(len(m)-1):
                ...
                if params.normalization.mode != 0:
                    ...
                    if std <= 0 or np.isnan(std):
                        img[n[i]:n[i+1], m[j]:m[j+1]] = 127
                    else:
                        img[n[i]:n[i+1], m[j]:m[j+1]] = 127 + np.reshape(
                            np.round((pp - np.mean(pp)) / std),
                            (s, s),
                        )
                else:
    ...
    def enhanceContrast(self, D):
        ...
        for i in range(D.shape[0]):
            half_window = int(self.params.contrastEnhancement.R / 2)
            a = np.max((0, i - half_window))
            b = np.min((D.shape[0], i + half_window + 1))
            v = D[a:b, :]
    ...
    def getMatches(self, DD):
        ...
        for N in range(half_ds, DD.shape[1] - half_ds):
            ...
            half_window = int(self.params.matching.Rwindow / 2)
            window = np.arange(
                np.max((0, min_idx - half_window)),
                np.min((len(score), min_idx + half_window)),
            )
